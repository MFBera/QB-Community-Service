...
local kamu = data.kamu
	...
	if xPlayer.PlayerData.job.name == "police" then
		...
		if tPlayer then
			...
			tPlayer.Functions.SetMetaData("communityservice", count)
			TriggerClientEvent("QBCore:Notify", src, "Kamu hizmeti verildi!")
		else
			TriggerClientEvent("QBCore:Notify", source, "Böyle Bir Oyuncu Yok!", "error")
				end
	end
end)

QBCore.Commands.Add("kamubitir", "", {{name="id", help="Player ID"}}, true, function(source, args) -- ad, yardım, argümanlar, argümanlar gerekli, sonuna kadar izin
