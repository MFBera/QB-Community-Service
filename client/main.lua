...
AddEventHandler('qb-communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and not communityServiceFinished do
		...
		if mesafe > 45 then
			...
			QBCore.Functions.Notify("Kaçamazsın ENAYİ. Kamu hizmetiniz uzatıldı. ÇALIŞ LAN KÖLE! Temizle şehiri")
		end
		Citizen.Wait(20000)
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if actionsRemaining > 0 and isSentenced then
			time = 1
			draw2dText("~b~" .." " ..QBCore.Shared.Round(actionsRemaining).. ' ~b~ ~s~ Hizmetinizi tamamlamadan önce tamamlamanız gereken daha fazla işlem var.', { 0.70, 0.955 } )
				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						if actionsRemaining == 0 then
							QBCore.Functions.Notify("Kamunu Bitirdin. Aferin LEN!", "success")
